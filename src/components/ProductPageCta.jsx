...
const ResoundifyHeroSection = () => {
    return (
        ...
        >
            {/* Left side: Image container with responsive sizing - 40% width on desktop */}
            {/* Added `flex-shrink-0` to prevent unintended shrinking */}
            <div className="relative w-full md:w-2/5 flex items-center justify-center py-8 px-4 md:py-30 md:px-0 flex-shrink-0">
            ...
            {/* Right side: Text content - 60% width on desktop for more breathing room */}
            {/* Responsive alignment: text-center on mobile, text-left on larger screens */}
            {/* Added `flex-shrink-0` to prevent unintended shrinking */}
            <div className="w-full md:w-3/5 relative z-20 flex flex-col items-center md:items-start justify-center p-4 md:p-8 text-white text-center md:text-left flex-shrink-0">
                <h2
                ...
                <p className="text-base sm:text-lg text-white mb-4 leading-relaxed p-1">
                    Explore our cutting-edge AV products designed to elevate your space. From professional sound systems to innovative visual displays, find the perfect solution for your needs.
                </p>
                {/* Integrated GradientButton with responsive width */}
                <GradientButton text="Contact Us!" href="/contact" className="w-full md:max-w-[240px] max-w-[160px] md:max-h-[100px] max-h-[60px] md:mt-0 mt-5" />
            </div>
